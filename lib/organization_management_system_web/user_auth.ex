defmodule OrganizationManagementSystemWeb.UserAuth do
  ...
  @remember_me_cookie "_organization_management_system_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      OrganizationManagementSystemWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      OrganizationManagementSystemWeb.Endpoint.broadcast(
  ...
  the `current_scope`:
      ...
      defmodule OrganizationManagementSystemWeb.PageLive do
        use OrganizationManagementSystemWeb, :live_view

        on_mount {OrganizationManagementSystemWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{OrganizationManagementSystemWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_authenticated_super_user, _params, session, socket) do
    socket = mount_current_scope(socket, session)
  ...
  def on_mount(:require_authenticated_org_creator, _params, session, socket) do
    socket = mount_current_scope(socket, session)
